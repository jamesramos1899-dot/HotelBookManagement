        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Room features, bed type, view, etc."
          ...
          <div className="flex gap-3 pt-4">
            ...
            <button
              type="submit"
              disabled={loading}
              className="flex-1 py-3 bg-gradient-to-r from-cyan-500 to-purple-500 rounded-xl font-bold text-white disabled:opacity-50"
            >
...
const HotelModal = ({ show, onClose, onSuccess, editingHotel }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      Swal.fire(
        "Error",
        err.response?.data?.error || "Failed to add hotel",
        "error",
      );
    }
    setLoading(false);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
      <div className="bg-slate-900 rounded-3xl p-8 max-w-2xl w-full border border-white/10 shadow-2xl max-h-[80vh] overflow-y-auto">
        <div className="flex justify-between items-center mb-6">
          <h3 className="text-2xl font-bold">
            {editingHotel ? "Edit Hotel" : "Add Your Hotel"}
          </h3>
          <button
            onClick={onClose}
            className="p-2 hover:bg-white/10 rounded-lg"
...
const HotelAdminDashboard = ({ user, onLogout }) => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (hotelsRes.success) {
        foundHotel = hotelsRes.data.find(
          (h) => h.owner === user?._id || h.createdBy === user?._id,
        );
      ...
      if (bookingsRes.success) {
        const hotelBookings = foundHotel
          ? bookingsRes.data.filter(
              (b) =>
                b.hotel?._id === foundHotel._id || b.hotel === foundHotel._id,
            )
          : [];
  ...
  const DashboardView = () => (
    <div className="space-y-6">
      ...
      <div className="grid md:grid-cols-2 gap-6">
          ...
          <div className="space-y-3">
            ...
            <div className="w-full bg-white/10 rounded-full h-2">
              <div
                ...
                style={{
                  width: `${stats.totalBookings > 0 ? Math.round((stats.confirmedBookings / stats.totalBookings) * 100) : 0}%`,
                }}
              />
            </div>
            ...
            <div className="w-full bg-white/10 rounded-full h-2">
              <div
                ...
                style={{
                  width: `${stats.totalBookings > 0 ? Math.round((stats.cancelledBookings / stats.totalBookings) * 100) : 0}%`,
                }}
              />
            </div>
          </div>
        ...
        <div className="bg-white/5 backdrop-blur-xl rounded-2xl p-6 border border-white/10">
          <h3 className="text-lg font-bold mb-4 flex items-center gap-2">
            <PhilippinePeso className="w-5 h-5 text-green-400" /> Revenue
            Analytics
          </h3>
          ...
          {[...bookings].sort((a, b) => new Date(b.checkInDate) - new Date(a.checkInDate)).slice(0, 5).map((booking) => (
            ...
            >
              ...
              <div className="text-right">
                <span
                  className={`text-xs px-2 py-1 rounded ${booking.status === "confirmed" ? "bg-green-500/20 text-green-400" : booking.status === "pending" ? "bg-yellow-500/20 text-yellow-400" : "bg-red-500/20 text-red-400"}`}
                >
                  {booking.status}
                </span>
        ...
        <div className="space-y-6">
          ...
          <div className="text-center py-12 bg-white/5 rounded-2xl border border-white/10">
            ...
            <button
              ...
              className="px-6 py-3 bg-gradient-to-r from-cyan-500 to-purple-500 rounded-xl font-bold text-white hover:shadow-lg transition-all"
            >
              Add Your Hotel
            </button>
          </div>
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="bg-white/5 rounded-lg p-3">
                  ...
                  <p
                    className={`font-medium ${room.isAvailable !== false ? "text-green-400" : "text-red-400"}`}
                  >
                ...
                <div className="flex-1">
                  ...
                  <div className="grid grid-cols-4 gap-4 mb-4 text-sm">
                    ...
                    <div>
                      ...
                      <span
                        className={`px-2 py-1 rounded-full text-xs ${booking.status === "confirmed" ? "bg-green-500/20 text-green-400" : booking.status === "pending" ? "bg-yellow-500/20 text-yellow-400" : "bg-red-500/20 text-red-400"}`}
                      >
                        {booking.status}
                      </span>
                    </div>
  ...
  const ReportsView = () => {
    const generatePDF = () => {
      ...
      doc.text(
        `Confirmed: ${stats.confirmedBookings} | Cancelled: ${stats.cancelledBookings}`,
        14,
        86,
      );
                  ...
                  <div className="flex-1 flex items-center justify-center">
                    <div className="text-center">
                      ...
                      <p className="text-gray-400">
                        Select a guest conversation to reply
                      </p>
                    </div>
                  </div>
                ) : (
                  <>
                    ...
                    >
                      {adminChatMessages.map((msg, idx) => (
                        <div
                          key={idx}
                          className={`flex flex-col ${msg.senderRole === "hotel_admin" ? "items-end" : "items-start"}`}
                        >
                          <p className="text-xs text-gray-500 mb-1 px-1">
                            {msg.senderRole === "hotel_admin"
                              ? "You"
                              : activeGuestName}
                          </p>
                          <div
                            className={`max-w-xs px-4 py-2 rounded-2xl text-sm ${msg.senderRole === "hotel_admin" ? "bg-cyan-500/30 text-white" : "bg-white/10 text-gray-200"}`}
                          >
                            <p>{msg.message}</p>
